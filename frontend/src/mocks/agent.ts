 ...
 * Correlated with K8s nodes and Infrastructure VMs
 */
...
// Helper functions for agent mock
 ...
 */
function generateVMAgent(vmIndex: number): AgentInfo {
...
/**
 * Backend-synced seed agents (4 agents matching backend seed data)
 * IDs and core data must remain stable to correlate with backend seeds.
 */
const SEED_AGENTS: AgentInfo[] = [
  {
    id: "agent-prod-server-01",
    name: "Production Server 1",
    version: "1.7.0",
    status: "online",
    lastSeen: Date.now() - randomInt(0, 60000),
    labels: { env: "production", datacenter: "us-east-1" },
    systemInfo: {
      hostname: "prod-server-01.telemetryflow.id",
      os: "linux",
      platform: "Amazon Linux",
      platformFamily: "rhel",
      platformVersion: "2023",
      kernelVersion: "5.15.0-1052-aws",
      kernelArch: "x86_64",
      virtualizationSystem: "nitro",
      virtualizationRole: "guest",
      hostId: "agent-prod-server-01",
      uptime: randomInt(86400, 2592000),
      bootTime: Date.now() - randomInt(86400000, 2592000000),
      procs: randomInt(150, 400),
      timezone: "UTC",
      timezoneOffset: 0,
    ...
    metrics: {
      ...
      network: [
        {
          interface: "eth0",
          bytesRecv: randomInt(5e9, 50e9),
          bytesSent: randomInt(5e9, 50e9),
          packetsRecv: randomInt(5e6, 50e6),
          packetsSent: randomInt(5e6, 50e6),
          errIn: 0,
          errOut: 0,
          dropIn: 0,
          dropOut: 0,
    ...
    host: {
      type: "vm",
      vmId: "agent-prod-server-01",
      vmName: "prod-server-01.telemetryflow.id",
      provider: "aws",
      region: "us-east-1",
  ...
  },
  {
    ...
    labels: { env: "production", datacenter: "us-east-1" },
    systemInfo: {
      hostname: "prod-server-02.telemetryflow.id",
      os: "linux",
      platform: "Amazon Linux",
      platformFamily: "rhel",
      platformVersion: "2023",
    ...
    host: {
      type: "vm",
      vmId: "agent-prod-server-02",
      vmName: "prod-server-02.telemetryflow.id",
      provider: "aws",
      region: "us-east-1",
      instanceType: "m5.xlarge",
    },
  ...
  {
    id: "agent-staging-server-01",
    name: "Staging Server",
    version: "1.7.0",
    status: "online",
    lastSeen: Date.now() - randomInt(0, 60000),
    labels: { env: "staging", datacenter: "us-east-1" },
    systemInfo: {
      hostname: "staging-server-01.telemetryflow.id",
      os: "linux",
      platform: "Amazon Linux",
      platformFamily: "rhel",
      platformVersion: "2023",
      ...
      virtualizationSystem: "nitro",
      virtualizationRole: "guest",
      hostId: "agent-staging-server-01",
      uptime: randomInt(86400, 2592000),
      bootTime: Date.now() - randomInt(86400000, 2592000000),
      procs: randomInt(100, 300),
      timezone: "UTC",
    ...
    metrics: {
      ...
      network: [
        {
          interface: "eth0",
          bytesRecv: randomInt(1e9, 10e9),
          bytesSent: randomInt(1e9, 10e9),
          packetsRecv: randomInt(1e6, 10e6),
          packetsSent: randomInt(1e6, 10e6),
          errIn: 0,
          errOut: 0,
          dropIn: 0,
          dropOut: 0,
    ...
    host: {
      type: "vm",
      vmId: "agent-staging-server-01",
      vmName: "staging-server-01.telemetryflow.id",
      provider: "aws",
      region: "us-east-1",
  ...
  {
    id: "agent-otel-collector-prod",
    name: "OTEL Collector - Production",
    version: "0.92.0",
    status: "online",
    lastSeen: Date.now() - randomInt(0, 60000),
    labels: { env: "production", type: "collector" },
    config: {
      receivers: ["otlp"],
    ...
    systemInfo: {
      hostname: "otel-collector-prod.telemetryflow.id",
      os: "linux",
      platform: "Amazon Linux",
      platformFamily: "rhel",
      platformVersion: "2023",
      ...
      virtualizationSystem: "nitro",
      virtualizationRole: "guest",
      hostId: "agent-otel-collector-prod",
      uptime: randomInt(86400, 2592000),
      bootTime: Date.now() - randomInt(86400000, 2592000000),
      procs: randomInt(50, 150),
      timezone: "UTC",
      timezoneOffset: 0,
    },
    metrics: {
      cpu: {
        usage: randomFloat(30, 60),
        cores: 4,
        threads: 8,
        model: "Intel(R) Xeon(R) Platinum 8375C CPU @ 2.90GHz",
        frequency: 2900,
      ...
      network: [
        {
          interface: "eth0",
          bytesRecv: randomInt(10e9, 100e9),
          bytesSent: randomInt(10e9, 100e9),
          packetsRecv: randomInt(10e6, 100e6),
          packetsSent: randomInt(10e6, 100e6),
          errIn: 0,
          errOut: 0,
          dropIn: 0,
          dropOut: 0,
    ...
    host: {
      type: "vm",
      vmId: "agent-otel-collector-prod",
      vmName: "otel-collector-prod.telemetryflow.id",
      provider: "aws",
      region: "us-east-1",
      instanceType: "m5.xlarge",
    },
 ...
 * The first 4 agents are backend-synced seed agents, followed by dynamically
 * generated K8s node agents and VM agents.
...
export function generateAgents(additionalVMs: number = 10): AgentInfo[] {
  const agents: AgentInfo[] = [...SEED_AGENTS];
  let agentIndex = 0;
 ...
 */
export function generateAgentTimeSeries(duration: number = 3600000) {
  const now = Date.now();
