...
// Node dimensions
const NODE_RADIUS_WS = 50;
const NODE_RADIUS_TENANT = 40;
...
const graphData = computed(() => {
  ...
  // Get unique workspaces that have tenants
  const wsIds = new Set(props.tenants.map((t) => t.workspaceId));
  const relevantWs = props.workspaces.filter((w) => wsIds.has(w.id));

  // Add workspace nodes
  relevantWs.forEach((ws) => {
...
function calculateNodePositions(
  ...
  if (layout.value === "layered") {
    const TENANT_SPACING = NODE_SPACING_X * 0.85;

    // Calculate width needed for each workspace based on its tenants
    const wsWidths: number[] = [];
    wsNodes.forEach((ws) => {
      const wsTenants = tenantNodes.filter((t) => t.parentId === ws.id);
      const tenantWidth = Math.max(
        NODE_SPACING_X,
        wsTenants.length * TENANT_SPACING,
      );
      wsWidths.push(tenantWidth);
    });

    // Calculate total width and starting position
    const totalWidth = wsWidths.reduce((sum, w) => sum + w, 0);
    let currentX = (canvasWidth - totalWidth) / 2;

    // Layer 1: Position workspaces based on their required widths
    wsNodes.forEach((node, i) => {
      const centerX = currentX + wsWidths[i] / 2;
    ...
    // Layer 2: Position tenants below their parent workspace
    currentX = (canvasWidth - totalWidth) / 2;
    wsNodes.forEach((ws, wsIndex) => {
      const wsTenants = tenantNodes.filter((t) => t.parentId === ws.id);
      const wsCenterX = currentX + wsWidths[wsIndex] / 2;

      if (wsTenants.length > 0) {
        const tenantWidth = wsTenants.length * TENANT_SPACING;
        const tenantStartX = wsCenterX - tenantWidth / 2 + TENANT_SPACING / 2;
      ...
      currentX += wsWidths[wsIndex];
    });
  } else if (layout.value === "force") {
    const canvasHeight = container?.clientHeight || 600;

    // Dynamic spacing constants
    const WS_SPACING = Math.max(NODE_SPACING_X, 280);
    const TENANT_BASE_RADIUS = 160;

    // Place workspaces in a row with more spacing
    const wsLayerWidth = wsNodes.length * WS_SPACING;
    const wsStartX = (canvasWidth - wsLayerWidth) / 2 + WS_SPACING / 2;
    ...
    // Place tenants in a circular pattern around their workspace with dynamic radius
    wsNodes.forEach((ws) => {
      const wsPos = positions.get(ws.id)!;
      const wsTenants = tenantNodes.filter((t) => t.parentId === ws.id);

      // Dynamic radius based on tenant count
      const tenantRadius = Math.max(TENANT_BASE_RADIUS, wsTenants.length * 45);

      wsTenants.forEach((tenant, i) => {
        const angle =
          (2 * Math.PI * i) / Math.max(wsTenants.length, 1) - Math.PI / 2;
        positions.set(tenant.id, {
          x: wsPos.x + tenantRadius * Math.cos(angle),
          y: wsPos.y + tenantRadius * Math.sin(angle) + 80,
        });
      });
    });
  } else {
...
function getNodeRadius(type: string): number {
  return type === "workspace" ? NODE_RADIUS_WS : NODE_RADIUS_TENANT;
}
...
function drawGraph(
  ...
  nodes.forEach((node) => {
    ...
    // Icon
    ctx.font = `${isWs ? 18 : 16}px Inter, sans-serif`;
    ...
    // Child count for workspaces
    if (isWs && node.childCount > 0) {
      ctx.fillStyle = borderColor;
      ctx.font = "bold 10px Inter, sans-serif";
      ctx.fillText(`${node.childCount} tenants`, pos.x, pos.y + 14);
    }
    ...
    ctx.fillText(name, pos.x, pos.y + radius + 16);

    // Slug
    ctx.fillStyle = colors.value.textSecondary;
    ctx.font = "9px Inter, sans-serif";
    const slug =
      node.slug.length > 18 ? node.slug.slice(0, 15) + "..." : node.slug;
    ctx.fillText(slug, pos.x, pos.y + radius + 29);

    // Type label
    ctx.fillStyle = borderColor;
    ctx.font = "8px Inter, sans-serif";
    ctx.fillText(node.type.toUpperCase(), pos.x, pos.y + radius + 41);
  });
...
function getWorkspaceName(node: TenancyGraphNode): string {
  if (node.type === "tenant") {
...
watch([() => props.workspaces, () => props.tenants, layout], () => {
      ...
      >
        <table class="tooltip-table">
          <tbody>
            ...
            <tr v-if="hoveredNode.type === 'tenant'">
              <td class="tooltip-label">Workspace:</td>
              <td class="tooltip-value">{{ getWorkspaceName(hoveredNode) }}</td>
            </tr>
            <tr v-if="hoveredNode.description">
              <td class="tooltip-label">Description:</td>
              <td class="tooltip-value">{{ hoveredNode.description }}</td>
            </tr>
            <tr v-if="hoveredNode.type === 'workspace'">
              <td class="tooltip-label">Tenants:</td>
      ...
      <template #header>
        <div class="modal-header">
          <div class="modal-title-section">
            ...
            <span class="modal-title">Tenant Hierarchy</span>
            <span class="modal-subtitle">Workspaces → Tenants</span>
          </div>
          ...
          >
            <table class="tooltip-table">
              <tbody>
                ...
                <tr v-if="modalHoveredNode.type === 'tenant'">
                  <td class="tooltip-label">Workspace:</td>
                  <td class="tooltip-value">
                    {{ getWorkspaceName(modalHoveredNode) }}
                  </td>
                </tr>
                ...
                <tr v-if="modalHoveredNode.type === 'workspace'">
                  <td class="tooltip-label">Tenants:</td>
                  <td class="tooltip-value">
